...
int prepareRuntime(const BitcodeInfoTy &info) {
  ...
  while (ss >> deplibs) {
    if (deplibs.substr(0,2) == "-l") {
      ...
      std::string bcname = "lib" + deplibs.substr(2) + ".bc";
      std::string full_lib_path = std::string(sysroot) + "/usr/lib/" + libname;
      std::string full_bc_path = std::string(lib_dir) + "/" + bcname;
      struct stat buf;
      // runtime found, ensure not to overwrite user's library
      if (stat(full_lib_path.c_str(),&buf) == 0 &&
          stat(full_bc_path.c_str(),&buf) != 0) {
        handleTask("cp -pn " + full_lib_path + " " + lib_dir + "/" + libname);
      }
    }
  }
  return 0;
