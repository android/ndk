...
enum Command {
  ...
  CMD_COMPILE,
  CMD_LINK,
  CMD_LINK_POSTCMD, // Linking needs something AFTER obj files
};
...
private:
  int parseLDFlags(const std::string&);
...
protected:
  ...
  std::string mExecutableToolsPath[(unsigned)CMD_LINK_POSTCMD+1];
...
public:
  ...
  virtual void cleanupPre() const {}
  virtual void cleanupPost() const {}
  void prepare() {
    prepareBitcodes();
...
protected:
  void runCmd(std::string cmd, bool dump_error = false);
};
