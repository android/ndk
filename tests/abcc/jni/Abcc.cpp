...
int BitcodeInfo::parseLDFlags(const std::string &orig_str) {
  ...
  while (ss >> str) {
    ...
    if (str == "-o") {
      ss >> str;  // Ignore on-hsto output path
      continue;
    }
    if (str == "-soname") {
      ss >> str;
    ...
    if (str.size() > 2 &&
        str.substr(0, 2) == "-l") {
      mLDLibs.push_back(str.substr(2));
      continue;
    }
  ...
  for (std::list<std::string>::const_iterator i = mLDLibs.begin(),
       e = mLDLibs.end(); i != e; ++i) {
    mLDLibsStr += " -l" + *i;
  }
  return 0;
  ...
  while (!mSonameMap.empty()) {
    ...
    for (; i != e; ++i) {
      ...
      if (bc.mLDLibs.empty()) {
        ...
        // Add ldlibs
        cmd += " " + mExecutableToolsPath[(unsigned)CMD_LINK_POSTCMD];
        cmd += " " + mGlobalLDLibs;
        cmd += " " + bc.mLDLibsStr;
        if (bc.mShared)
          cmd += std::string(" ") + mSysroot + "/usr/lib/crtend_so.o";
        else
          cmd += std::string(" ") + mSysroot + "/usr/lib/crtend_android.so";
        runCmd(cmd);
        if (returnCode() != RET_OK)
          return;
...
void BitcodeCompiler::runCmd(std::string cmd, bool dump_error) {
