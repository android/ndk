...
void DeviceBitcodeCompiler::cleanupPost() {
  ...
  cmd = "chmod 0644 " + mWorkingDir + "/compile_result";
  runCmd(cmd);

  if (mRet != RET_OK) {
    cmd = "touch " + mWorkingDir + "/compile_error";
    runCmd(cmd);
    cmd = "chmod 0644 " + mWorkingDir + "/compile_error";
    runCmd(cmd);
    cmd = "chmod 0644 " + mWorkingDir + "/compile_log";
    runCmd(cmd);
  } else {
...
void DeviceBitcodeCompiler::copyRuntime(const BitcodeInfo &info) {
  ...
  runCmd(std::string("chmod 0755 ") + mWorkingDir + "/libgabi++_shared.so");
...
#if VERBOSE
  ReturnCode code = mRet;
  runCmd(std::string("chmod 0755 ") + path);  // Fix permission for future use
  mRet = code;  // Ignore error when change mode
#else
