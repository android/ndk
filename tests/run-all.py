...
class Failure(TestResult):
    ...
    def __repr__(self):
        return 'FAIL {}: {}'.format(self.test_name, self.message)
...
class Success(TestResult):
    ...
    def __repr__(self):
        return 'PASS {}'.format(self.test_name)
...
class Skipped(TestResult):
    ...
    def __repr__(self):
        return 'SKIP {}: {}'.format(self.test_name, self.reason)
...
def run_awk_test(out_dir, test_dir):
    ...
    for test_case in glob.glob(os.path.join(test_dir, '*.in')):
        ...
        golden_out_path = re.sub(r'\.in$', '.out', test_case)
        if not os.path.isfile(golden_out_path):
            return Failure(test_name,
                           'missing output: {}'.format(golden_out_path))
        return run_awk_test_case(out_dir, test_name, script, test_case,
                                 golden_out_path)
...
def run_device_test(out_dir, test_dir, build_flags, abi, platform):
    ...
    try:
        ...
        for case in test_cases:
            ...
            if result != 0:
                return Failure('.'.join([test_name, case]), '\n'.join(out))
        return Success(test_name)
    finally:
...
def run_tests(out_dir, test_dir, test_func):
    for dentry in os.listdir(test_dir):
        ...
        if os.path.isdir(path):
            try:
                print(test_func(out_dir, path))
            except RuntimeError as ex:
                print(Failure(os.path.basename(dentry), ex))
...
def main():
    ...
    if 'awk' in suites:
        run_tests(out_dir, 'awk', run_awk_test)
    if 'build' in suites:
        run_tests(out_dir, 'build', my_run_build_test)
    if 'samples' in suites:
        run_tests(out_dir, '../samples', my_run_build_test)
    if 'device' in suites:
        run_tests(out_dir, 'device', my_run_device_test)
