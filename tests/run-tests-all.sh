...
if [ "$SYSTEM" = "linux-x86x" ] ; then
    ...
    NDK_TOOLCHAIN_VERSION=4.4.3 ./run-tests.sh --full --wine
    dump "### Running windows clang 3.1 full tests"
    NDK_TOOLCHAIN_VERSION=clang3.1 ./run-tests.sh --full --wine
fi
