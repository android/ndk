...
if [ "$ABI" = "unknown" ]; then
    ...
    if [ -z "$UNKNOWN_ARCH" ]; then
        dump "ERROR: No unknown arch support"
        exit 1
    fi

    # Filter out those using gnustl or gabi++ only
    TESTS_DIR=$ROOTDIR/tests
    TESTS="`find $TESTS_DIR  | xargs grep 'import-module.*cxx-stl/gnu-libstdc++' | grep -v 'run-tests' | awk -F '/' '{print $9}' | xargs`"
    UNKNOWN_INCOMPATIBLE_TESTS="$UNKNOWN_INCOMPATIBLE_TESTS $TESTS"
    TESTS="`find $TESTS_DIR | xargs grep 'import-module.*cxx-stl/gabi++' | grep -v 'run-tests' | awk -F '/' '{print $9}' | xargs`"
    UNKNOWN_INCOMPATIBLE_TESTS="$UNKNOWN_INCOMPATIBLE_TESTS $TESTS"
    TESTS="`find $TESTS_DIR  | xargs grep 'APP_STL.*gnustl' | grep -v 'run-tests' | awk -F '/' '{print $9}' | xargs`"
    UNKNOWN_INCOMPATIBLE_TESTS="$UNKNOWN_INCOMPATIBLE_TESTS $TESTS"
...
case $ABI in
    ...
    armeabi|armeabi-v7a|x86|mips)
        ...
        ;;
    unknown)
        NDK_BUILD_FLAGS="$NDK_BUILD_FLAGS APP_ABI=unknown"
        NDK_BUILD_FLAGS="$NDK_BUILD_FLAGS APP_STL=stlport_static STLPORT_FORCE_REBUILD=true"
        ;;
    *)
...
if is_testable device; then
    ...
    run_device_test ()
    {
        ...
        if [ "$ABI" = "unknown" ] && [ -d "$BUILD_DIR/`basename $TEST`/libs" ]; then
            cd $BUILD_DIR/`basename $TEST`/libs && ln -s $UNKNOWN_ARCH $CPU_ABI
        fi
        ...
        if [ "$ABI" = "unknown" ]; then # on-the-fly on-device compilation
    ...
    if [ "$SKIP_TESTS" = "yes" ] ; then
        ...
        for DEVICE in $ADB_DEVICES; do
            ...
            for CPU_ABI in $CPU_ABIS; do
                if [ "$ABI" = "default" -o "$ABI" = "$CPU_ABI" -o "$ABI" = "unknown" ] ; then
