...
class TestRunner(object):
    ...
    def _fixup_expected_failure(self, test, result, config, bug):
        if result.failed():
            return ExpectedFailure(test.name, config, bug)
        elif result.passed():
            return UnexpectedSuccess(test.name, config, bug)
    ...
    def _run_test(self, test, out_dir, test_filters):
        ...
        return [self._fixup_expected_failure(test, r, config, bug)
                for r in results]
...
class TestConfig(object):
    ...
    def from_test_dir(cls, test_dir):
        ...
        if os.path.exists(path):
            ...
            metadata = {}
        if 'broken' in metadata and 'bug' not in metadata['broken']:
            raise ValueError('broken tests must have a bug number')
        return cls(metadata)
