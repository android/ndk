...
def build_vulkan(out_dir, dist_dir, args):
    ...
    copies = [
        {
            'source_dir': vulkan_root_dir + '/vulkan',
            'dest_dir': 'vulkan/src',
            'files': [
              'vk-generate.py',
              'vk_helper.py',
        ...
        {
            'source_dir': vulkan_root_dir + '/vulkan/loader',
        ...
        {
            'source_dir': vulkan_root_dir + '/glslang',
    ...
    src = os.path.join(vulkan_root_dir, 'vulkan/buildAndroid')
    dst = os.path.join(vulkan_path, 'build-android')
    shutil.rmtree(dst, 'true')
    shutil.copytree(src, dst,
                  ignore=default_ignore_patterns)

    print('vulkan_path: ' + vulkan_path)
    build_support.merge_license_files(
        os.path.join(vulkan_path, 'NOTICE'), [
        os.path.join(vulkan_root_dir, 'vulkan/LICENSE.txt')])
    ...
    dst = os.path.join(out_dir, 'vulkan/validation-layers')
    build_support.make_package('validation-layers', dst, dist_dir)
