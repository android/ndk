...
def pack_binutils(toolchain, out_dir, binutils_path):
    archive_name = os.path.join(out_dir, 'binutils-' + toolchain)
    archive_base_path = os.path.relpath(binutils_path, out_dir)
    shutil.make_archive(archive_name, 'bztar',
                        root_dir=os.path.realpath(out_dir),
                        base_dir=archive_base_path)
...
def build_binutils(out_dir, args):
    ...
    for arch in arches:
        ...
        binutils_out = os.path.join(out_dir, 'binutils', toolchain)
        ...
        for file_name in get_binutils_files(triple, has_gold):
            install_file(file_name, toolchain_path, binutils_out)
        pack_binutils(toolchain, out_dir, binutils_out)
...
def build_gcc_libs(out_dir, args):
    ...
    for arch in arches:
        ...
        install_dir = os.path.join(out_dir, 'prebuilt/gcclibs')
        ...
        for libgcc in libgccs:
            ...
            archive_base_path = os.path.relpath(install_dir, out_dir)
            shutil.make_archive(archive_name, 'bztar',
                                root_dir=os.path.realpath(out_dir),
                                base_dir=archive_base_path)
