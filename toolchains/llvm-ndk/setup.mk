...
TARGET_AR       := $(TOOLCHAIN_PREBUILT_ROOT)/bin/llvm-ndk-ar
...
# Use portable headers + ARM's headers
TARGET_C_INCLUDES := $(NDK_ROOT)/sources/android/pndk/include \
...
TARGET_pndk_release_CFLAGS :=

# When building for debug, compile everything as pndk.
TARGET_pndk_debug_CFLAGS := $(TARGET_pndk_release_CFLAGS)
...
$(call set-src-files-target-cflags, $(__debug_sources), $(TARGET_pndk_debug_CFLAGS)) \
$(call set-src-files-target-cflags, $(__release_sources),$(TARGET_pndk_release_CFLAGS)) \
$(call set-src-files-text,$(LOCAL_SRC_FILES),BC$(space)$(space)$(space)) \
...
$(PRIVATE_LD) \
    $(TARGET_LDFLAGS) \
    $(filter-out %.o %.so,$(PRIVATE_LINKER_OBJECTS_AND_LIBRARIES)) \
    $(filter-out %.o %.so,$(PRIVATE_LDLIBS)) \
...
$(PRIVATE_AR) \
    ...
    $(filter-out %.o %.so,$(PRIVATE_AR_OBJECTS))
endef
