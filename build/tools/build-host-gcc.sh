...
have to define DARWIN_TOOLCHAIN / DARWIN_CFLAGS / CXXFLAGS / LDFLAGS in
your environment first (note: this feature is highly experimental).
...
# $1: host prefix, no trailin slash (e.g. i686-linux-android)
            ...
            case $BUILD_OS in
                ...
                *)
                    if [ -z "$DARWIN_TOOLCHAIN" ]; then
                        ...
                        dump "Please define DARWIN_TOOLCHAIN to name it. You can also use"
                        dump "DARWIN_CFLAGS/CXXFLAGS/LDFLAGS for additional compiler/linker flags."
                        dump "For example:"
                        dump ""
                        dump "   DARWIN_TOOLCHAIN=i686-apple-darwin11"
                        dump "   DARWIN_CFLAGS=\"-isysroot ~/toolchain4/sdks/MacOSX10.7.sdk -mmacosx-version-min=10.4 -DMAXOSX_DEPLOYEMENT_TARGET=10.4\""
                        dump "   DARWIN_CXXFLAGS=\$DARWIN_CFLAGS"
                        dump "   DARWIN_LDFLAGS=\"-Wl,-syslibroot, -mmacosx-version-min=10.4"
                        dump "   export DARWIN_TOOLCHAIN DARWIN_CFLAGS DARWIN_CXXFLAGS DARWIN_LDFLAGS"
                        dump ""
                        dump "Hint: Consider using gen-toolchain-wrapper.sh to embed all the cflags/ldflags automatically."
                        dump ""
                        exit 1
                    fi
                    try_host_prefix "$DARWIN_TOOLCHAIN" "$DARWIN_CFLAGS"
                    if [ -z "$HOST_FULLPREFIX" ]; then
                    ...
                    HOST_CFLAGS=$DARWIN_CFLAGS
                    HOST_CXXFLAGS=$DARWIN_CXXFLAGS
                    HOST_LDFLAGS=$DARWIN_LDFLAGS
                    ;;
            esac
            ;;
        ...
        windows-x86_64)
            ...
            case $BUILD_OS in
                linux)
                    ...
                    # versions of Ubuntu (i.e. it fails at building trivial
                    # programs).
...
build_host_gcc_core ()
{
    ...
    # .ctors/.dtors on Android. Note that upstream Linux GLibc is no doing
    # the same.
    ARGS=$ARGS" --enable-init-fini-array"
