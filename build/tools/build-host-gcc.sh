...
Unfortunately, Darwin toolchains can only be built on Darwin machines at
the moment.
...
Windows toolchain that targets Android ARM binariers) will force the generation
...
EOF
    log -n "Checking compiler code generation... "
    ...
    if [ -z "$HOST_FULLPREFIX" ]; then
        ...
        if [ -f "$GCC" ]; then
            if check_compiler "$GCC" "$@"; then
                ...
                log "Using host gcc: $GCC $@"
            else
                log "Ignoring broken host gcc: $GCC $@"
            fi
        fi
    fi
}
