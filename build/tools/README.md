...
runtime (with _very_ limited capabilites), a.k.a. /system/lib/libstdc++.so. The
...
separate from $NDK is because it is not primarly developed in the open.
...
NDK git directory (and is spefically listed in $NDK/.gitignore) but of its final
installation.
...
By default, this only rebuilds the host prebuilds for the current host system.
