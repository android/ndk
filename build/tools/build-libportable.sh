...
build_libportable_libs_for_abi ()
{
    ...
    builder_end
  # extract __wrap functions and create a *.wrap file of "-Wl,--wrap=symbol"
    nm -a $DSTDIR/libportable.a | grep -r __wrap | awk '{print $3}' | sed '/^$/d' | \
        sed 's/^__wrap//g' | sort | awk '{printf "-Wl,--wrap=%s\n",$1}' > "$DSTDIR/libportable.wrap"
}
