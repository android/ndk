...
def get_all_packages(host, arches):
    # List of (package_name, arch_specific, host_specific).
    packages = [
        'binutils-{arch}-{host}',
        'build',
        'cpufeatures',
        'gcclibs-{arch}',
        'gdb-multiarch-7.10-{host}',
        'gdbserver-{arch}',
        'gnustl-4.9',
        'gtest',
        'libcxx',
...
def check_for_packages(path, packages):
    for package in packages:
...
def main():
    ...
    check_for_packages(args.package_dir, packages)
