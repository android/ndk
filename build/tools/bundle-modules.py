...
def make_package(package_dir, packages, host, out_dir, temp_dir):
    date = datetime.date.today()
    release_name = 'android-ndk-{:%Y%m%d}'.format(date)
    ...
    shutil.make_archive(package_path, 'bztar', package_dir, extract_dir)


class ArgParser(argparse.ArgumentParser):
    def __init__(self):
        super(ArgParser, self).__init__(
            description='Install NDK modules to a directory for use. If '
                        '--package is used, instead generates a monolithic '
                        'NDK package.')
        ...
        self.add_argument(
            ...
            help='Build a tarball instead of extracting to OUT_DIR.')
        self.add_argument(
...
def main():
    ...
    if os.path.exists(args.out_dir) and not args.repackage:
        if args.force:
    ...
    if args.repackage:
        ...
        try:
            make_package(args.package_dir, packages, args.host, args.out_dir,
                         package_dir)
        finally:
