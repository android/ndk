...
test "$ARCH_INC" != "$ARCH" && LLVM_VERSION=$DEFAULT_LLVM_VERSION && STL=stlport && ABI=$(convert_arch_to_abi $ARCH | tr ',' '\n' | tail -n 1)
...
dump_extra_compile_commands () {
  ...
  if [ -z "$HOST_EXE" ]; then
    ...
    echo ':check_done'
    echo '%~dp0\\python'$HOST_EXE' %~dp0\\ndk-bc2native.py --sysroot=%~dp0\\..\\sysroot --abi='$ABI' --platform='$PLATFORM' --file %output% %output%'
  fi
}
