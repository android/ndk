...
class ArgParser(build_support.ArgParser):
    def __init__(self):  # pylint: disable=super-on-old-class
        ...
        self.add_argument(
            '--symbols', action='store_true', dest='symbols', default=False,
            help='Include layer symbol files (they are large).')
        self.add_argument(
            '--no-symbols', action='store_false', dest='symbols',
            help='Do not include layer symbol files.')
        self.add_argument(
...
def main(args):
    ...
    vulkan_dir = os.path.join(args.out_dir, 'vulkan/src')
    build_dir = os.path.join(vulkan_dir, 'build-android')
    package_out = os.path.join(args.out_dir, 'vulkan/vulkan-validation-layers')
    lib_out = os.path.join(package_out, 'jniLibs')
    ...
    build_cmd = [
        ...
        'APP_STL=' + stl,
        'NDK_APPLICATION_MK=' + os.path.join(build_dir, 'jni',
                                             'Application.mk'),
        'NDK_OUT=' + obj_out,
    ...
    if args.symbols is True:
        ...
        for properties in copies:
            ...
            for m_dir in properties['dirs']:
                src = os.path.join(source_dir, m_dir)
                dst = os.path.join(dest_dir, m_dir)
                shutil.rmtree(dst, 'true')
                shutil.copytree(src, dst,
                                ignore=default_ignore_patterns)
            for m_file in properties['files']:
                src = os.path.join(source_dir, m_file)
                dst = os.path.join(dest_dir, m_file)
                shutil.copyfile(src, dst)

    dst = os.path.join(args.out_dir, 'vulkan', stl)
