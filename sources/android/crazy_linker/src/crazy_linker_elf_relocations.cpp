...
bool ElfRelocations::Init(const ElfView* view, Error* error) {
  ...
  bool uses_rela_relocations = false;
  ...
  for (; dyn.HasNext(); dyn.GetNext()) {
    ...
    switch (tag) {
      ...
      case DT_REL:
        ...
        uses_rela_relocations |= tag == DT_RELA;
        break;
      case DT_RELASZ:
      case DT_RELSZ:
        RLOG("  %s size=%d\n",
        ...
        uses_rela_relocations |= tag == DT_RELASZ;
        break;
      case DT_PLTGOT:
  ...
  if ((relocations_type_ == DT_REL && uses_rela_relocations) ||
      (relocations_type_ == DT_RELA && !uses_rela_relocations)) {
    *error = "Unsupported DT_REL/DT_RELA/DT_PLTREL combination in dyn section";
    return false;
  }
