...
struct crazy_context_t {
 public:
  crazy_context_t() : load_address(0), file_offset(0), error(), search_paths() {
    ResetSearchPaths();
  }
...
void crazy_context_t::ResetSearchPaths() {
  search_paths.Reset();
  const char* env = crazy::GetEnv("LD_LIBRARY_PATH");
  if (env && env[0])
    search_paths.AddPaths(env);
}
...
crazy_status_t crazy_library_use_relro_sharing(crazy_library_t* library,
  ...
  if (!wrap->UseSharedRelro(
           relro_start, relro_size, relro_fd, &context->error)) {
    return CRAZY_STATUS_KO;
  }
...
crazy_status_t crazy_library_find_symbol(crazy_library_t* library,
  ...
  *symbol_address = wrap->LookupSymbol(symbol_name);
  return (*symbol_address == NULL) ? CRAZY_STATUS_KO : CRAZY_STATUS_OK;
}
