...
def main(args):
    ...
    build_cmd = [
        ...
        # No-op for non-ARM, and we'll get the thumb libs in the next step if
        # necessary.
        'LOCAL_ARM_MODE=arm',
    ...
    # The static libraries are install to NDK_OUT, not NDK_LIB_OUT, so we need
    # to install them to our package directory.
    for abi in abis:
        install_dir = os.path.join(lib_out, abi)
        shutil.copy2(os.path.join(obj_out, 'local', abi, 'libc++_static.a'),
                     install_dir)

    if 'arm' in arches:
        arm_abis = build_support.arch_to_abis('arm')
        thumb_obj_out = os.path.join(args.out_dir, 'libcxx/thumb/obj')
        thumb_lib_out = os.path.join(args.out_dir, 'libcxx/thumb/libs')
        thumb_flags = [
            'APP_ABI=' + ' '.join(arm_abis),
            'LOCAL_ARM_MODE=thumb',
            'NDK_OUT=' + thumb_obj_out,
            'NDK_LIBS_OUT=' + thumb_lib_out,
        ]

        print('Building thumb libc++ for ABIs: {}'.format(', '.join(arm_abis)))
        subprocess.check_call(build_cmd + thumb_flags)

        for abi in arm_abis:
            thumb_install_dir = os.path.join(lib_out, abi, 'thumb')
            os.makedirs(thumb_install_dir)
            shutil.copy2(
                os.path.join(thumb_obj_out, 'local', abi, 'libc++_static.a'),
                thumb_install_dir)
            shutil.copy2(
                os.path.join(thumb_lib_out, abi, 'libc++_shared.so'),
                thumb_install_dir)
