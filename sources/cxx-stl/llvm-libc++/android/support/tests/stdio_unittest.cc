...
TEST(stdio,snprintf) {
  ...
  static const char kWideString[] = "Hello, World";
  EXPECT_EQ(12, snprintf(char_buff, char_buff_len, "%s", kWideString));
  EXPECT_EQ(12, snprintf(char_buff, 13, "%s", kWideString));

  EXPECT_EQ(12, snprintf(char_buff, 12, "%s", kWideString));
  EXPECT_EQ(L'\0', char_buff[11]);
  EXPECT_EQ(12, snprintf(char_buff, 1, "%s", kWideString));
